...
class QuadraticEquation
{
    ...
    static void FormatingPrint(double x1, double x2)
    {
        ...
        if (x1 == x2)
        {
            Console.WriteLine(checkX1 ? "x{1} = x{2} = {0:0.0}" : "x{1} = x{2} = {0}", x1, index1.ToLower(), index2.ToLower());
        }
        else
        {
            Console.Write(checkX1 ? "x{1} = {0:0.0}; " : "x{1} = {0}; ", x1, index1.ToLower());
            Console.WriteLine(checkX1 ? "x{1} = {0:0.0}" : "x{1} = {0}", x2, index2.ToLower());
        }
