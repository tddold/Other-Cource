...
class Unicode_Character
{
    static void Main()
    {
        ...
        Console.WriteLine("{0} ({1}) --> 0x{2:X4} ({3}) --> '\\u{4:X4}' ({5}) --> '{6}' ({7})", decValue, decValue.GetType().Name, intValue, intValue.GetType().Name, Convert.ToInt16(number), number.GetType().Name, number, number.GetType().Name);        
    }
}